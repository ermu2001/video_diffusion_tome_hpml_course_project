...
def train(args: DictConfig, accelerator: Accelerator) -> None:
    ...
    teacher_unet = UNet2DConditionModel.from_pretrained(
        args.model.pretrained_teacher_model, subfolder="unet", revision=args.model.teacher_revision
    )
    ...
    if args.train.cast_teacher_unet:
    ...
    if accelerator.is_main_process:
        unet = accelerator.unwrap_model(unet)
        unet.save_pretrained(args.status.output_dir)
        lora_state_dict = get_peft_model_state_dict(unet, adapter_name="default")
        StableDiffusionPipeline.save_lora_weights(os.path.join(args.status.output_dir, "unet_lora"), lora_state_dict)
